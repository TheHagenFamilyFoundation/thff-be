...
module.exports = {
  ...
  async migrate(req, res) {
    ...
    sails.log('migrating organization infos');

    let organizationInfos = await OrganizationInfo.find().populate('organization');
    const length = organizationInfos.length;
    sails.log('length', length);

    const newOrganizationInfos = [];
    organizationInfos.forEach((organizationInfo) => {

      if (organizationInfo?.organization?.id) {
        const newOrganizationInfo = {
          _id: organizationInfo.id,
          createdAt: organizationInfo.createdAt,
          updatedAt: organizationInfo.updatedAt,
          organizationInfoID: organizationInfo.organizationInfoID,
          legalName: organizationInfo.legalName,
          yearFounded: organizationInfo.yearFounded,
          currentOperatingBudget: organizationInfo.currentOperatingBudget,
          director: organizationInfo.director,
          phone: organizationInfo.phone,
          contactPerson: organizationInfo.contactPerson,
          contactPersonTitle: organizationInfo.contactPersonTitle,
          contactPersonPhoneNumber: organizationInfo.contactPersonPhoneNumber,
          ...
          organization: organizationInfo.organization.id,
          validOrgInfo: organizationInfo.validOrgInfo

        };

        newOrganizationInfos.push(newOrganizationInfo);
      }
    });
    const afterLength = newOrganizationInfos.length;
    sails.log('afterLength', afterLength);
    return res.status(200).json(newOrganizationInfos);
  }
