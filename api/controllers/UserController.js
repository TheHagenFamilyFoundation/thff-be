...
module.exports = {
  ...
  async create(req, res) {
    ...
    if (!userfound) {
      if (req.body.password !== req.body.confirmPassword) {
        return res
          .status(401)
          .json({ err: "Password doesn't match, What a shame!" });
      }
  ...
  async UserNameExists(req, res) {
    ...
    const userfound = await sails.helpers.user.userNameExists(
      req.query.username
    );
  ...
  async EmailExists(req, res) {
    ...
    // helpers
    // user folder
    const emailfound = await sails.helpers.user.userEmailExists(
      req.query.email
    );
      ...
      if (emailfound) {
        if (emailfound.length > 0) {
          ...
          User.update(
            ...
            return res
              .status(200)
              .json({ resetCodeCreated: true, resetCode: text });
          });
        } else {
          sails.log("email is not found");
  ...
  changeEmail(req, res) {
    ...
    User.find({
      ...
      User.update(
        ...
        return res
          .status(200)
          .json({ change: true, message: "Change Email Successful" });
      });
    });
  }, // end of changeEmail
  ...
  updateName(req, res) {
    ...
    User.find({
      ...
      User.update(
        ...
        return res
          .status(200)
          .json({ change: true, message: "Update Name Successful" });
      });
    });
  }, // end of updateName
  ...
  async getUserCounts(req, res) {
    ...
    if (req.query.org) {
      const org = await Organization.findOne({ id: req.query.org }).populate(
        "users"
      );
      sails.log("org", org);
